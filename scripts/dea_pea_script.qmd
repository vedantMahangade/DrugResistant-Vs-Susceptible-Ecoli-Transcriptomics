...
```{r}
go_enrichment <- compareCluster(gene = gene_list_by_antibiotic,
                                fun = 'enrichGO',
                                OrgDb = org.EcK12.eg.db,
                                keyType = "ENTREZID",
                                #universe = rownames(geo_data),
                                pvalueCutoff = 0.25,
                                qvalueCutoff = 0.25,
                                pAdjustMethod="BH")
...
```{r}
GOresultsTable <- go_enrichment@compareClusterResult
```

```{r}
go_dot_plot <- dotplot(go_enrichment, x = "Cluster",
        color = "p.adjust",
        showCategory = 10,
        split = "category",
        font.size = 8,
        ...
        label_format = 40)
ggsave(
  filename = "analysis/summary/go_dot_plot.png",
  plot = go_dot_plot,
  width = 6, height = 8, dpi = 300
)
```
...
```{r}
GOnetwork_plot <- cnetplot(go_enrichment,
                           # showCategory=10,
